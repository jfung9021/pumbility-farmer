...
def inclusive_percentile_skill(ratings: Sequence[float]) -> dict[str, Any]:
    """Average actual observations inside the inclusive linear 25th–50th interval."""
    values = np.asarray(ratings, dtype=float)
    values = values[np.isfinite(values)]
    if not len(values):
        return {"q25Skill": None, "q50Skill": None, "meanSkill": None, "selectedCount": 0}
    q25, q50 = np.quantile(values, [0.25, 0.50], method="linear")
    selected = values[(values >= q25) & (values <= q50)]
    return {
        "q25Skill": float(q25),
        "q50Skill": float(q50),
...
def build_tier_metrics(
    ...
    """Return public aggregates, calibrated within exact mode/official-level folders.
...
def tier_metric_method(folder_references: Mapping[str, float]) -> dict[str, Any]:
    return {
        ...
        "clearing": {
            ...
            "percentiles": [0.25, 0.50],
            "percentileMethod": "linear, inclusive boundaries and ties",
            "calibration": "official level + 0.5 + mean selected skill - folder median selected skill",
