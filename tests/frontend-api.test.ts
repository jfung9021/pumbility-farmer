...
test("tier demo calibrates final folder medians and supports cross-level estimates and missing components", () => {
  ...
  for (const charts of [payload.singles, payload.doubles]) {
    ...
    for (const chart of charts) {
      ...
      if (clearing.estimatedDifficulty !== null) {
        assert.equal(clearing.estimatedDifficulty, chart.level + 0.5 + clearing.meanSkill! - clearing.folderReferenceSkill!);
      }
...
test("Top 50 export uses a bounded canonical poster route and centered control", async () => {
  const [page, route, renderer, css, regularFont, boldFont, symbolsFont] = await Promise.all([
    readFile(path.join(process.cwd(), "app", "recommendations", "page.tsx"), "utf8"),
    readFile(path.join(process.cwd(), "app", "exports", "top50", "route.tsx"), "utf8"),
    readFile(path.join(process.cwd(), "lib", "top50-poster-render.ts"), "utf8"),
    readFile(path.join(process.cwd(), "app", "globals.css"), "utf8"),
  ...
  const posterCardStart = route.indexOf("function PosterCard");
  const posterCardEnd = route.indexOf("export async function GET", posterCardStart);
  assert.ok(posterCardStart >= 0 && posterCardEnd > posterCardStart);
  const posterCard = route.slice(posterCardStart, posterCardEnd);
  const nextConfig = await readFile(path.join(process.cwd(), "next.config.js"), "utf8");
  ...
  assert.match(route, /JACKET_RENDER_MAX_WIDTH = 700/);
  assert.match(route, /JACKET_RENDER_MAX_HEIGHT = 393/);
  assert.match(route, /\.resize\(JACKET_RENDER_MAX_WIDTH, JACKET_RENDER_MAX_HEIGHT, \{[\s\S]*fit: "inside"[\s\S]*withoutEnlargement: true/);
  assert.doesNotMatch(route, /JACKET_RENDER_(?:WIDTH|HEIGHT) = (?:212|108)/);
  assert.doesNotMatch(route, /scalePosterTree|POSTER_RENDER_SCALE|new ImageResponse/);
  assert.doesNotMatch(route, /from ["']next\/og["']/);
  assert.match(route, /renderTop50Poster\(/);
  assert.match(route, /topScores\.slice\(0, 50\)/);
  assert.match(nextConfig, /serverExternalPackages: \["@resvg\/resvg-js"\]/);
  assert.match(renderer, /import satori[^\n]*from "satori"/);
  assert.match(renderer, /renderAsync as renderSvgAsync[^\n]*from "@resvg\/resvg-js"/);
  assert.match(renderer, /await satori\(/);
  assert.match(renderer, /await renderSvgAsync\(svg, \{/);
  assert.match(renderer, /mode: "zoom"[\s\S]*value: TOP50_EXPORT_SCALE/);
  assert.match(renderer, /loadSystemFonts: false/);
  assert.match(renderer, /renderedUi\.width !== TOP50_EXPORT_WIDTH[\s\S]*renderedUi\.height !== TOP50_EXPORT_HEIGHT/);
  assert.match(renderer, /const uiPixels = renderedUi\.pixels/);
  assert.match(renderer, /\.composite\(\[[\s\S]*input: uiPixels[\s\S]*channels: 4/);
  assert.match(renderer, /\.raw\(\)[\s\S]*\.png\(\{ compressionLevel: PNG_COMPRESSION_LEVEL \}\)/);
  assert.match(renderer, /\.png\(/);
  assert.doesNotMatch(renderer, /ImageResponse|arrayBuffer\(\)|density:/);
  assert.match(route, /height: TOP50_POSTER_HEIGHT/);
  assert.match(route, /width: TOP50_POSTER_WIDTH/);
  assert.match(posterCard, /height: 166/);
  assert.match(posterCard, /height: 108/);
  assert.match(posterCard, /background: difficultyColor/);
  assert.match(route, /Generated by \{TOP50_EXPORT_SITE\}/);
  assert.match(posterCard, /fontSize: 20[\s\S]*fontWeight: 800[\s\S]*lineHeight: 1[\s\S]*\{score\.difficulty\}/);
  assert.match(posterCard, /fontSize: 17[\s\S]*fontWeight: 900[\s\S]*lineHeight: 1[\s\S]*>\#\{rank\}/);
  assert.match(posterCard, /fontSize: 17[\s\S]*fontWeight: 800[\s\S]*height: 28[\s\S]*lineHeight: 1[\s\S]*>\{score\.level\}/);
  assert.match(posterCard, /alignItems: "center"[\s\S]*height: 23[\s\S]*overflow: "hidden"[\s\S]*padding: "0 4px"[\s\S]*fontSize: 14[\s\S]*lineHeight: 1\.2[\s\S]*minWidth: 0[\s\S]*overflow: "hidden"[\s\S]*whiteSpace: "nowrap"[\s\S]*\{score\.songName\}/);
  assert.match(route, /fontSize: 26[\s\S]*height: 54[\s\S]*>PF/);
  assert.match(route, /fontSize: 18[\s\S]*lineHeight: 1\.1[\s\S]*TOP 50 PUMBILITY/);
  assert.match(route, /fontSize: 41[\s\S]*lineHeight: 1\.05[\s\S]*textOverflow: "ellipsis"[\s\S]*\{playerName\}/);
  assert.match(route, /fontSize: 17[\s\S]*lineHeight: 1\.15[\s\S]*textOverflow: "ellipsis"[\s\S]*\{modeLabel\(modeValue\)\}/);
  assert.match(route, /fontSize: 16[\s\S]*lineHeight: 1[\s\S]*PUMBILITY/);
  assert.match(route, /fontSize: 60[\s\S]*\{total\.integer\}/);
  assert.match(route, /fontSize: 34[\s\S]*\{total\.fraction\}/);
  assert.match(route, /import \{ pumbilityProgress \} from "\.\.\/\.\.\/\.\.\/lib\/pumbility-progress"/);
  assert.match(route, /const OVERALL_RANK_EMBLEM_FILENAMES = Array\.from\([\s\S]*pumbility_\$\{String\(rungIndex\)\.padStart\(2, "0"\)\}\.webp/);
  assert.match(route, /const overallRankEmblemDataUrlCache = new Map<number, Promise<string>>\(\)/);
  assert.match(route, /function overallRankEmblemDataUrl\(rungIndex: number\): Promise<string>/);
  assert.match(route, /const rankEmblemDataUrl = modeValue === "overall"[\s\S]*pumbilityProgress\("overall", pumbilityValue\)\.rungIndex[\s\S]*: null/);
  assert.match(route, /\{rankEmblemDataUrl \? \([\s\S]*<img[\s\S]*height="88"[\s\S]*src=\{rankEmblemDataUrl\}[\s\S]*marginRight: 12[\s\S]*width="88"[\s\S]*\) : null\}[\s\S]*alignItems: "baseline"[\s\S]*height: 60/);
  assert.doesNotMatch(route, /scored chart/);
  assert.match(route, /fontSize: 17[\s\S]*justifyContent: "space-between"[\s\S]*Generated by \{TOP50_EXPORT_SITE\}/);
  assert.match(route, /height: 145/);
  assert.match(route, /height: 1750/);
  const exportLayoutConstant = (name: string): number => {
    const match = route.match(new RegExp(`const ${name} = (\\d+)`));
    assert.ok(match, `${name} must be a numeric export-layout constant`);
    return Number(match[1]);
  };
  assert.ok(exportLayoutConstant("POSTER_RESULT_FONT_SIZE") > 15);
  assert.equal(exportLayoutConstant("POSTER_RESULT_GAP"), 5);
  assert.ok(exportLayoutConstant("POSTER_PLATE_FONT_SIZE") > 11);
  assert.ok(exportLayoutConstant("POSTER_PUMBILITY_FONT_SIZE") > 15);
  assert.ok(exportLayoutConstant("POSTER_RESULT_GAP") > 0);
  assert.ok(exportLayoutConstant("POSTER_PUMBILITY_GAP") > 0);
  assert.match(posterCard, /alignItems: "baseline"[\s\S]*gap: POSTER_PUMBILITY_GAP[\s\S]*justifyContent: "space-between"[\s\S]*padding: "5px 4px 8px"/);
  const metadataGroupStart = posterCard.indexOf(
    '<div style={{ alignItems: "baseline", display: "flex", flex: "0 1 auto", gap: POSTER_RESULT_GAP, minWidth: 0 }}>',
  );
  const pumbilityStart = posterCard.indexOf(
    '<div style={{ color: "#c8ff2e", display: "flex", flex: "0 0 auto", fontSize: POSTER_PUMBILITY_FONT_SIZE',
    metadataGroupStart,
  );
  assert.ok(metadataGroupStart >= 0 && pumbilityStart > metadataGroupStart);
  const metadataGroup = posterCard.slice(metadataGroupStart, pumbilityStart);
  assert.equal((metadataGroup.match(/<div style=/g) ?? []).length, 4);
  assert.match(metadataGroup, /fontSize: POSTER_RESULT_FONT_SIZE[\s\S]*\{scoreValue\}[\s\S]*fontSize: POSTER_RESULT_FONT_SIZE[\s\S]*\{score\.grade \|\| "—"\}[\s\S]*fontSize: POSTER_PLATE_FONT_SIZE[\s\S]*\{score\.plateCode \|\| "—"\}/);
  assert.equal((metadataGroup.match(/fontSize: POSTER_RESULT_FONT_SIZE/g) ?? []).length, 2);
  assert.doesNotMatch(metadataGroup, /marginLeft: "auto"/);
  assert.match(posterCard.slice(pumbilityStart), /\{pumbility\}/);
  assert.match(posterCard, /whiteSpace: "nowrap"/);
  assert.match(route, /fontFamily: '\"Noto Sans\", \"Noto Sans Symbols 2\", sans-serif'/);
  assert.match(route, /JACKET_TOTAL_TIMEOUT_MS = 8_000/);
  assert.match(route, /renderTop50Poster\([\s\S]*name: "Noto Sans"[\s\S]*name: "Noto Sans Symbols 2"/);
  assert.match(route, /JACKET_CACHE_MAX_ENTRIES = 128/);
  assert.match(route, /JACKET_CACHE_TTL_MS = 10 \* 60_000/);
  assert.match(route, /loadCachedJacketData\(url/);
  assert.match(route, /POSTER_CACHE_MAX_BYTES = 64 \* 1024 \* 1024/);
  assert.match(route, /POSTER_CACHE_MAX_ENTRIES = 4/);
  assert.match(route, /POSTER_CACHE_TTL_MS = 5 \* 60_000/);
  assert.match(route, /createHash\("sha256"\)[\s\S]*layoutVersion: POSTER_LAYOUT_VERSION/);
  assert.match(route, /cacheStatus: cached\.pending \? "coalesced" : "hit"/);
  assert.match(route, /return \{ cacheStatus: "miss", promise \}/);
  for (const timing of [
    "poster-cache",
    "jacket-fetch-normalize",
    "jacket-resize",
    "satori",
    "resvg",
    "composite",
    ...
    assert.match(route, new RegExp(`${timing.replace("-", "\\-")};`));
  }
  assert.match(route, /"Server-Timing": serverTimingParts\.join/);
  assert.match(route, /"X-Top50-Poster-Cache": cachedPoster\.cacheStatus/);
  for (const font of [regularFont, boldFont, symbolsFont]) {
...
test("Top 50 export helpers enforce dimensions, modes, origins, and jacket allowlisting", () => {
  assert.equal(TOP50_POSTER_WIDTH, 1156);
  assert.equal(TOP50_POSTER_HEIGHT, 2048);
  assert.equal(TOP50_EXPORT_SCALE, 2);
  assert.equal(TOP50_EXPORT_WIDTH, 2312);
  assert.equal(TOP50_EXPORT_HEIGHT, 4096);
  assert.equal(TOP50_EXPORT_WIDTH, TOP50_POSTER_WIDTH * TOP50_EXPORT_SCALE);
  assert.equal(TOP50_EXPORT_HEIGHT, TOP50_POSTER_HEIGHT * TOP50_EXPORT_SCALE);
...
test("Top 50 export benchmark is deterministic and network-independent", async () => {
  const [benchmark, route] = await Promise.all([
    readFile(path.join(process.cwd(), "scripts", "benchmark_top50_export.ts"), "utf8"),
    readFile(path.join(process.cwd(), "app", "exports", "top50", "route.tsx"), "utf8"),
  ]);

  assert.match(benchmark, /const SCORE_COUNT = 50/);
  assert.match(benchmark, /const CARD_WIDTH = 212/);
  assert.match(benchmark, /const CARD_HEIGHT = 166/);
  assert.match(benchmark, /"1,000,000"/);
  assert.match(benchmark, /"SSS\+"/);
  assert.match(benchmark, /"PG"/);
  for (const [constant, expected] of Object.entries({
    POSTER_FALLBACK_FONT_SIZE: 20,
    POSTER_CARD_BADGE_FONT_SIZE: 17,
    POSTER_TITLE_FONT_SIZE: 14,
    POSTER_PF_FONT_SIZE: 26,
    POSTER_HEADER_LABEL_FONT_SIZE: 18,
    POSTER_PLAYER_FONT_SIZE: 41,
    POSTER_HEADER_META_FONT_SIZE: 17,
    POSTER_TOTAL_LABEL_FONT_SIZE: 16,
    POSTER_TOTAL_INTEGER_FONT_SIZE: 60,
    POSTER_TOTAL_FRACTION_FONT_SIZE: 34,
    POSTER_FOOTER_FONT_SIZE: 17,
  })) {
    assert.match(benchmark, new RegExp(`const ${constant} = ${expected}`));
  }
  for (const constant of [
    "POSTER_RESULT_FONT_SIZE",
    "POSTER_RESULT_GAP",
    "POSTER_PLATE_FONT_SIZE",
    "POSTER_PUMBILITY_FONT_SIZE",
    "POSTER_PUMBILITY_GAP",
  ]) {
    const pattern = new RegExp(`const ${constant} = (\\d+)`);
    const benchmarkValue = benchmark.match(pattern)?.[1];
    const routeValue = route.match(pattern)?.[1];
    assert.ok(benchmarkValue && routeValue, `${constant} must exist in route and benchmark`);
    assert.equal(benchmarkValue, routeValue);
  }
  assert.match(benchmark, /flex: "0 1 auto"[\s\S]*gap: POSTER_RESULT_GAP/);
  assert.match(benchmark, /const POSTER_RESULT_GAP = 5/);
  assert.equal((benchmark.match(/fontSize: POSTER_RESULT_FONT_SIZE/g) ?? []).length, 2);
  assert.doesNotMatch(benchmark, /marginLeft: "auto"/);
  assert.match(benchmark, /worstCaseMetadata: "1,000,000 SSS\+ PG"/);
  assert.match(benchmark, /alignItems: "center"[\s\S]*height: 23[\s\S]*padding: "0 4px"[\s\S]*fontSize: POSTER_TITLE_FONT_SIZE[\s\S]*lineHeight: 1\.2[\s\S]*minWidth: 0[\s\S]*overflow: "hidden"[\s\S]*whiteSpace: "nowrap"/);
  assert.match(benchmark, /padding: "5px 4px 8px"/);
  assert.match(benchmark, /pumbilityProgress\("overall", FIXTURE_PUMBILITY\)/);
  assert.match(benchmark, /rankEmblemFilename = `pumbility_\$\{String\(rankProgress\.rungIndex\)\.padStart\(2, "0"\)\}\.webp`/);
  assert.match(benchmark, /height: "88"[\s\S]*src: rankEmblemDataUrl[\s\S]*marginRight: 12[\s\S]*width: "88"/);
  assert.doesNotMatch(benchmark, /scored charts/);
  assert.match(benchmark, /deliberately long title/);
  assert.match(benchmark, /fontSize: POSTER_PLAYER_FONT_SIZE[\s\S]*textOverflow: "ellipsis"[\s\S]*whiteSpace: "nowrap"/);
  assert.match(benchmark, /fontSize: POSTER_FOOTER_FONT_SIZE[\s\S]*Generated by pumbility-farmer\.vercel\.app/);
  assert.match(benchmark, /renderTop50Poster\(/);
  assert.match(benchmark, /metadata\.width !== TOP50_EXPORT_WIDTH/);
  assert.match(benchmark, /metadata\.height !== TOP50_EXPORT_HEIGHT/);
  assert.match(benchmark, /top50ExportFilename\("overall", FIXTURE_DATE\)/);
  assert.match(benchmark, /fixture: "jonathan-overall-local"/);
  assert.match(benchmark, /timings: result\.timings/);
  assert.doesNotMatch(benchmark, /\bfetch\(|https?:\/\//);
});
