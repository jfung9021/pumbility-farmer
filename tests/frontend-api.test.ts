...
test("Top 50 export uses a bounded canonical poster route and centered control", async () => {
  const [page, route, css, regularFont, boldFont, symbolsFont] = await Promise.all([
    readFile(path.join(process.cwd(), "app", "recommendations", "page.tsx"), "utf8"),
  ...
  assert.match(route, /sharp\(image,[\s\S]*\.resize\(JACKET_RENDER_WIDTH, JACKET_RENDER_HEIGHT, \{ fit: "cover" \}\)[\s\S]*\.jpeg\(\{ quality: 82 \}\)/);
  assert.match(route, /POSTER_RENDER_SCALE = 0\.75/);
  assert.match(route, /function scalePosterTree\(node: ReactNode\)/);
  assert.match(route, /height: POSTER_RENDER_HEIGHT,[\s\S]*width: POSTER_RENDER_WIDTH/);
  assert.match(route, /renderedPoster\.arrayBuffer\(\)[\s\S]*\.resize\(TOP50_EXPORT_WIDTH, TOP50_EXPORT_HEIGHT/);
  ...
  assert.match(route, /fonts: \[[\s\S]*name: "Noto Sans"[\s\S]*name: "Noto Sans Symbols 2"/);
